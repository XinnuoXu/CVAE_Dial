...
class Glove(object):
	...
	not_unk = 0
        ...
        return paragraph_vector, not_unk
    ...
    def paragraph_similarity(self, paragraph_1, paragraph_2):
        """
        Run a similarity query, retrieving number
        most similar words.
        """
        paragraph_vector_1, not_unk_1 = self.transform_paragraph(paragraph_1, ignore_missing=True)
        paragraph_vector_2, not_unk_2 = self.transform_paragraph(paragraph_2, ignore_missing=True)
	try:
            return 1 - cosine(paragraph_vector_1, paragraph_vector_2), not_unk_1, not_unk_2
        except:
            return -1, not_unk_1, not_unk_2
	
	'''
        dst = (np.dot(paragraph_vector_1, paragraph_vector_2)
               / np.linalg.norm(paragraph_vector_1)
               / np.linalg.norm(paragraph_vector_2))
        return dst, not_unk_1, not_unk_2
	'''
    ...
    def most_similar_paragraph(self, paragraph, number=5, **kwargs):
        ...
        paragraph_vector, not_unk = self.transform_paragraph(paragraph, **kwargs)
