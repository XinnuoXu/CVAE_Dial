...
$system_2_better_nist = 0;
for ($i = 0; $i < $num_samples; $i++) {
	if ($system1_nist[$i] > $system2_nist[$i]) {
		$system_1_better_nist++;
	}
        elsif ($system2_nist[$i] > $system1_nist[$i]){
		$system_2_better_nist++;
        }
}
$system_1_better_bleu = 0;
$system_2_better_bleu = 0;
for ($i = 0; $i < $num_samples; $i++) {
	if ($system1_bleu[$i] > $system2_bleu[$i]) {
		$system_1_better_bleu++;
	}
	elsif ($system2_bleu[$i] > $system1_bleu[$i]) {
		$system_2_better_bleu++;
	}
}
...
$system_2_better_nist_fraction = $system_2_better_nist / $num_samples;
$system_2_better_bleu_fraction = $system_2_better_bleu / $num_samples;

$level = 1.0 - $p;

if ($system_1_better_nist_fraction >= 0.5){
...
if ($system_1_better_bleu_fraction >= 0.5){
